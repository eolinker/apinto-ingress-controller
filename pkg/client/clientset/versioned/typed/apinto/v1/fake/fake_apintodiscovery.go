/*
Copyright The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	apintov1 "github.com/eolinker/apinto-ingress-controller/pkg/apis/apinto/v1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeApintoDiscoveries implements ApintoDiscoveryInterface
type FakeApintoDiscoveries struct {
	Fake *FakeApintoV1
	ns   string
}

var apintodiscoveriesResource = schema.GroupVersionResource{Group: "apinto.com", Version: "v1", Resource: "apintodiscoveries"}

var apintodiscoveriesKind = schema.GroupVersionKind{Group: "apinto.com", Version: "v1", Kind: "ApintoDiscovery"}

// Get takes name of the apintoDiscovery, and returns the corresponding apintoDiscovery object, and an error if there is any.
func (c *FakeApintoDiscoveries) Get(ctx context.Context, name string, options v1.GetOptions) (result *apintov1.ApintoDiscovery, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(apintodiscoveriesResource, c.ns, name), &apintov1.ApintoDiscovery{})

	if obj == nil {
		return nil, err
	}
	return obj.(*apintov1.ApintoDiscovery), err
}

// List takes label and field selectors, and returns the list of ApintoDiscoveries that match those selectors.
func (c *FakeApintoDiscoveries) List(ctx context.Context, opts v1.ListOptions) (result *apintov1.ApintoDiscoveryList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(apintodiscoveriesResource, apintodiscoveriesKind, c.ns, opts), &apintov1.ApintoDiscoveryList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &apintov1.ApintoDiscoveryList{ListMeta: obj.(*apintov1.ApintoDiscoveryList).ListMeta}
	for _, item := range obj.(*apintov1.ApintoDiscoveryList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested apintoDiscoveries.
func (c *FakeApintoDiscoveries) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(apintodiscoveriesResource, c.ns, opts))

}

// Create takes the representation of a apintoDiscovery and creates it.  Returns the server's representation of the apintoDiscovery, and an error, if there is any.
func (c *FakeApintoDiscoveries) Create(ctx context.Context, apintoDiscovery *apintov1.ApintoDiscovery, opts v1.CreateOptions) (result *apintov1.ApintoDiscovery, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(apintodiscoveriesResource, c.ns, apintoDiscovery), &apintov1.ApintoDiscovery{})

	if obj == nil {
		return nil, err
	}
	return obj.(*apintov1.ApintoDiscovery), err
}

// Update takes the representation of a apintoDiscovery and updates it. Returns the server's representation of the apintoDiscovery, and an error, if there is any.
func (c *FakeApintoDiscoveries) Update(ctx context.Context, apintoDiscovery *apintov1.ApintoDiscovery, opts v1.UpdateOptions) (result *apintov1.ApintoDiscovery, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(apintodiscoveriesResource, c.ns, apintoDiscovery), &apintov1.ApintoDiscovery{})

	if obj == nil {
		return nil, err
	}
	return obj.(*apintov1.ApintoDiscovery), err
}

// Delete takes name of the apintoDiscovery and deletes it. Returns an error if one occurs.
func (c *FakeApintoDiscoveries) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteActionWithOptions(apintodiscoveriesResource, c.ns, name, opts), &apintov1.ApintoDiscovery{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeApintoDiscoveries) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(apintodiscoveriesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &apintov1.ApintoDiscoveryList{})
	return err
}

// Patch applies the patch and returns the patched apintoDiscovery.
func (c *FakeApintoDiscoveries) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *apintov1.ApintoDiscovery, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(apintodiscoveriesResource, c.ns, name, pt, data, subresources...), &apintov1.ApintoDiscovery{})

	if obj == nil {
		return nil, err
	}
	return obj.(*apintov1.ApintoDiscovery), err
}
