...
func (r *services) list() []*v1.Service {
	r.mu.RLock()
	res := make([]*v1.Service, 0, len(r.data))
...
func (r *services) update(name string, service *v1.Service) string {
	r.mu.Lock()
	defer r.mu.Unlock()
	service.ID = r.genID(name)
	r.data[service.ID] = service
	return service.ID
}
func (r *services) create(service *v1.Service) string {
	r.mu.Lock()
	defer r.mu.Unlock()
	service.ID = r.genID(service.Name)
	r.data[service.ID] = service
	return service.ID
}
func (r *services) del(name string) (*v1.Service, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	v, ok := r.data[r.genID(name)]
	if !ok {
...
func runServiceServer(t *testing.T) *http.Server {
	ln, _ := nettest.NewLocalListener("tcp")
	httpSrv := &http.Server{
		Addr: ln.Addr().String(),
		Handler: &services{
			data: make(map[string]*v1.Service),
		},
	}
	go func() {
...
func TestService(t *testing.T) {
	...
	// test update
	cases[0].Upstream = "update@upstream"
